...
string cycle = "cwix19";
string authenticationUrl = "https://tide.act.nato.int/login?url=%2Fmediawiki%2Fcwix19%2Findex.php%2FCWIX_2019";
...
async Task<string[]> BuildORGUrls()
{
    ...
    string countryBaseUrl = schema["ORG"]!["IDENTIFIER_COUNTRY_url"]!.ToString();

    string[] countryUrls = Utils.ParseValueString(schema["ORG"]!["IDENTIFIER_COUNTRY_values"]!.ToString())
                                .Select(country => countryBaseUrl.Replace("{{COUNTRY_IDENTIFIER}}", country))
                                .ToArray();

    string[] countryPages = await Utils.Download(countryUrls, await Utils.GetClient(baseAddress, authenticationUrl, username, password));

    foreach (var page in countryPages)
    {
        result.AddRange(GetUrls(page));
    }

    return result.ToArray();

    IEnumerable<string> GetUrls(string countryPage)
    {
        var document = new HtmlDocument();

        document.LoadHtml(countryPage);

        var commandsNode = document.DocumentNode.SelectSingleNode("//div[text()='Commands']/..//div[@class='gallery']");

        return commandsNode.ChildNodes.Where(item => item.Name == "a").Select(anchor => $"{baseAddress}{anchor.Attributes["href"]?.Value}");
    }
}
