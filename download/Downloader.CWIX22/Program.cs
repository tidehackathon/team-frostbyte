...
string[] ttUrls = await BuildTTREPOUrls();
...
async Task<string[]> BuildTTREPOUrls()
{
    List<string> result = new List<string>();

    string baseUrl = schema["TT"]!["url"]!.ToString();

    (int Start, int End)[] ranges = Utils.ParseRangeString(schema["TT"]!["IDENTIFIER_range_REPO"]!.ToString());
    ...
    foreach (var range in ranges)
    {
        string[] urls = Enumerable.Range(range.Start, range.End - range.Start).Select(identifier =>
        {
            ...
            string url = baseUrl.Replace("{{IDENTIFIER}}", id).Replace("{{IDENTIFIER_PREFIX}}", "REPO");
...
async Task<string[]> BuildTTUrls()
{
    ...
    foreach (var range in ranges)
    {
        string[] urls = Enumerable.Range(range.Start, range.End - range.Start).Select(identifier =>
        {
            ...
            string url = baseUrl.Replace("{{IDENTIFIER}}", id).Replace("{{IDENTIFIER_PREFIX}}", "TT"); ;
