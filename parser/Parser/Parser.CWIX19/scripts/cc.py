...
def extract_data_from_file(file_path):
    ...
    try:
        ...
        maturityTD=bodyContent.findAll('table', class_='wikitable')[1].findAll('td')[2]
        maturity = maturityTD.text.strip()
       
        banner=bodyContent.find('div',class_='banner-center').text.strip()
        ...
        descriptionP=bodyContent.find('h4',string='Description').parent.find_all('p')
        ...
        tag=bodyContent.find('div',string='Tasks').parent.find('div',class_='gallery')        
        ...
        interoperabilityachievements=bodyContent.find('div',id='Final_Report_-__28Execution_29').find('div',string='Interoperability Achievements').next_sibling.next_sibling.text

        interoperabilityachievements=[ x.strip() for x in interoperabilityachievements.split('\n')] 
        interoperabilityachievements=[x for x in interoperabilityachievements if x]
        
        
        interoperabilityimprovements=[]
        try:

            interoperabilityimprovements=bodyContent.find('div',id='Final_Report_-__28Execution_29').find('div',string='Interoperability Improvements').next_sibling.text

            interoperabilityimprovements=[ x.strip() for x in interoperabilityimprovements.split('\n')]
            interoperabilityimprovements=[x for x in interoperabilityimprovements if x]
        except:
            pass


        interoperabilitychallenges=bodyContent.find('div',id='Final_Report_-__28Execution_29').find('div',string='Interoperability Challenges').next_sibling.next_sibling.text

        interoperabilitychallenges=[ x.strip() for x in interoperabilitychallenges.split('\n')]
        interoperabilitychallenges=[x for x in interoperabilitychallenges if x!='']
        ...
        pathToSave='../../../../../data/normalized/cwix19/cc/'+fileId
...
extract_data_from_directory('../../../../../data/raw/cwix19/cc')
